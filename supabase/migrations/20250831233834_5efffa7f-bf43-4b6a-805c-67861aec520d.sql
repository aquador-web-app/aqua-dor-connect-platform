-- Add referral tracking during signup
ALTER TABLE public.profiles 
ADD COLUMN referred_by_code TEXT REFERENCES public.profiles(referral_code);

-- Add automatic barcode generation trigger if not already exists
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'trigger_set_profile_barcode') THEN
        CREATE TRIGGER trigger_set_profile_barcode
            BEFORE INSERT ON public.profiles
            FOR EACH ROW EXECUTE FUNCTION public.set_profile_barcode();
    END IF;
END $$;
